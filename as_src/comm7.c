...
valu_t load(item_t* ip) {
  ...
  if ((ip->i_type & S_TYP) == S_UND || (ip->i_type & S_COM)) {
    if (pass == PASS_3) {
      if (relonami != 0) serror("relocation error");
...
int store(item_t* ip, valu_t val) {
  ...
  typ = ip->i_type & S_TYP;
  if ((typ -= S_MIN) >= 0) val -= sect[typ].s_base;
#else
  if ((ip->i_type & S_TYP) == S_UND) return (0);
#endif
...
int combine(int typ1, int typ2, int op) {
  switch (op) {
    case '+':
      if (typ1 == S_ABS) return (typ2);
      if (typ2 == S_ABS) return (typ1);
      break;
    case '-':
      if (typ2 == S_ABS) return (typ1);
      ...
      break;
    case '>':
      if (typ1 == S_ABS && typ2 == S_ABS) return (S_ABS);
      if (((typ1 & ~S_DOT) == (typ2 & ~S_DOT) && typ1 != S_UND) ||
          (typ1 == S_ABS) || (typ2 == S_ABS))
        return (S_ABS | S_VAR);
      break;
    default:
      if (typ1 == S_ABS && typ2 == S_ABS) return (S_ABS);
  ...
  if (pass != PASS_1) serror("illegal operator");
  return (S_UND);
}
...
int small(int fitsmall, int gain) {
  ...
  if (nbits == BITCHUNK) {
    ...
    if (pbittab[bitindex] == 0) return (0);
  }
  bit = 1 << (nbits & 7);
...
void ffreopen(char* s, FILE* f) {
  if (freopen(s, "r", f) == NULL) fatal("can't reopen %s", s);
}

FILE* ffcreat(char* s) {
  FILE* f;

  if ((f = fopen(s, "w")) == NULL) fatal("can't create %s", s);
  return (f);
}
...
FILE* fftemp(char* path, char* tail) {
  ...
  if ((dir = getenv("TMPDIR")) == NULL) dir = tmp_dir;
...
void nosect(void) { fatal("no sections"); }

void wr_fatal(void) { fatal("write error"); }
...
void nofit(void) {
  if (pass == PASS_3) warning("too big");
}
