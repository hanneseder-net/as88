...
#ifdef ASLD
  if (archmode && --archsize < 0) return (0);
#endif
  if ((c = getc(input)) == EOF) return (0);
  if (isascii(c) == 0) fatal("non-ascii character");
#ifdef LISTING
  if (listflag & 0440) putc(c, listfile);
#endif
  return (c);
}
...
static int induo(int c) {
  static short duo[] = {
      ('=' << 8) | '=', OP_EQ, ('<' << 8) | '>', OP_NE, ('<' << 8) | '=', OP_LE,
      ('>' << 8) | '=', OP_GE, ('<' << 8) | '<', OP_LL, ('>' << 8) | '>', OP_RR,
      ('|' << 8) | '|', OP_OO, ('&' << 8) | '&', OP_AA,
  };
...
static int innumber(int c) {
  ...
  radix = 20;
  do {
    if (--radix < 0) fatal("number too long");
    if (isupper(c)) c += ('a' - 'A');
    *p++ = c;
    c = nextchar();
  } while (isalnum(c));
  ...
  *p = '\0';
  ...
  if (radix != 16 && (c == 'f' || c == 'b')) return (infbsym(num));
  yylval.y_valu = 0;
  while ((c = *p++)) {
    if (c > '9') c -= ('a' - '9' - 1);
    c -= '0';
    if ((unsigned)c >= radix) serror("digit exceeds radix");
...
item_t *item_search(char *p) {
  ...
  for (h = hash(p); h < H_TOTAL; h += H_SIZE) {
    ip = hashtab[h];
    while (ip != 0) {
      if (strcmp(p, ip->i_name) == 0) goto done;
      ip = ip->i_next;
    }
  }
done:
